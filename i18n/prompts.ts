...
Im Anschluss an Ihre Kritik geben Sie drei konkrete, umsetzbare Verbesserungsvorschläge. Dies könnten die Hinzufügung eines bestimmten Kunstwerks (Name, Künstler und Begründung), die Entfernung eines Werks, das das Thema schwächt, oder eine Neuordnung der Werke zur Schaffung einer stärkeren Erzählung sein. Antworten Sie in geschliffenem, professionellem Deutsch.`,
        ...
        audioGuide: (title, curator, artworkDataJson) => `Sie sind ein professioneller Produzent von Audioguides. Ihre Aufgabe ist es, ein vollständiges Audioguide-Skript für eine virtuelle Ausstellung mit dem Titel "${title}", kuratiert von ${curator}, zu erstellen. Der Tonfall sollte fesselnd, zugänglich und dennoch äußerst informativ sein – wie ein erstklassiger Museums-Podcast. Das Skript muss für das gesprochene Wort verfasst sein und klare, prägnante Sätze verwenden.
Das Skript muss enthalten:
1.  **Eine fesselnde Einleitung:** Eine 60-90-sekündige Einleitung, die den Hörer willkommen heißt, das Thema der Galerie vorstellt und einen evokativen Ton für das Erlebnis setzt.
2.  **Einzelne Kunstwerk-Segmente:** Erstellen Sie für jedes in den untenstehenden JSON-Daten angegebene Kunstwerk ein 45-60-sekündiges narratives Segment. Jedes Segment sollte das Kunstwerk nicht nur beschreiben, sondern seine Geschichte erzählen, ein entscheidendes Detail hervorheben oder es mit dem übergeordneten Thema der Galerie verbinden.
Hier ist die Liste der Kunstwerke: ${artworkDataJson}. Das gesamte Skript muss in eloquentem, sendefähigem Deutsch verfasst sein.`,
        journal: (topic) => `Handeln Sie als wissenschaftlicher Mitarbeiter mit Expertise in Kunstgeschichte. Führen Sie eine umfassende Untersuchung zum Thema durch: "${topic}". Synthetisieren Sie Informationen aus Ihrer Wissensdatenbank und simulierten Websuchen, um einen detaillierten, gut strukturierten enzyklopädischen Eintrag zu erstellen. Ihre Antwort muss:
1.  Eine klare und prägnante Definition und einen Überblick über das Thema geben.
2.  Die historische Entwicklung, Schlüsselfiguren und wegweisende Werke erörtern.
3.  Die Kernkonzepte, Techniken oder philosophischen Grundlagen analysieren.
4.  Verschiedene Denkschulen, Debatten oder kritische Interpretationen zum Thema aufzeigen.
5.  Mit einer Zusammenfassung der nachhaltigen Wirkung und Relevanz abschließen.
Formatieren Sie die Antwort mit Markdown unter Verwendung von Überschriften (###) und Aufzählungspunkten (*) für maximale Klarheit und Lesbarkeit. Die gesamte Ausgabe sollte ein einziger Textblock auf Deutsch sein.`,
        chatSystemInstruction: (title, artist, description) => `Übernimm die Persona von 'Art-i-Fact', einem KI-gestützten Kunsthistoriker und kuratorischen Assistenten. Deine Persönlichkeit ist gelehrt, leidenschaftlich und aufschlussreich, wie ein erfahrener Museumskurator, der eine private Führung leitet. Du diskutierst das Kunstwerk "${title}" von ${artist}, das wie folgt beschrieben wird: "${description}". Dein Ziel ist es, eine faszinierende, tiefgehende Konversation zu führen. Biete proaktiv tiefe Einblicke, stelle Verbindungen zu größeren Kunstbewegungen her und stelle denkanstoßende Fragen, um die Neugier des Nutzers zu wecken. Sei kein passiver Antwortgeber, sondern ein enthusiastischer Führer. Antworte in eloquentem, gesprächigem Deutsch.`,
        ...
        similarArt: (artwork) => {
            ...
            return `Handeln Sie als Experte für Kunst-Recherche, der eine Suchanfrage erstellt. Ihr Ziel ist es, visuell und thematisch ähnliche Kunstwerke zu dem unten beschriebenen zu finden. Synthetisieren Sie die Kernelemente zu einer prägnanten, schlagkräftigen Anfrage, die für eine öffentliche Kunst-Datenbank (wie Wikimedia Commons oder die Suche einer Museumssammlung) geeignet ist. Berücksichtigen Sie die Kunstrichtung, das Sujet, den Stil und die Stimmung. Fügen Sie relevante alternative Begriffe oder Künstlernamen hinzu, falls zutreffend. Die Anfrage sollte in der Regel auf Englisch sein, da dies in Kunstdatenbanken oft zu besseren Ergebnissen führt.

Zum Beispiel, wenn das Quell-Kunstwerk "Die Sternennacht von Vincent van Gogh" ist, wäre eine gute Suchanfrage "Post-Impressionist night sky OR swirling clouds landscape".

Quell-Kunstwerk: ${context}.
Geben Sie NUR den optimierten Suchanfragetext zurück, ohne einleitende Sätze, Erklärungen oder Anführungszeichen.`;
        },
    }
};