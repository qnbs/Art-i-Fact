...
const ArtworkItemComponent: React.FC<ArtworkItemProps> = ({ artwork, onAdd, onViewDetails }) => {
  ...
  const handleCardClick = (e: React.MouseEvent<HTMLDivElement>) => {
    // Prevents firing onAdd when the details button is clicked
  ...
  return (
    ...
    >
      ...
      <button 
        ...
        className="details-button absolute top-2 right-2 bg-black/50 backdrop-blur-sm rounded-full p-2 text-white opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 transition-all hover:bg-amber-600/70"
