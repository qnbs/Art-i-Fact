...
export const ChatModal: React.FC<ChatModalProps> = ({ artwork, language }) => {
  ...
  useEffect(() => {
    const initChat = async () => {
      setIsLoading(true);
      const chatSession = await gemini.startArtChat(
        artwork,
        appSettings,
        language,
      );
      ...
      const initialResponse = await chatSession.sendMessage({
        message:
          "Give me a brief, fascinating insight about this piece to start our conversation.",
      });
    ...
    <div className="flex flex-col h-[60vh]">
      <div className="flex-grow overflow-y-auto p-1 pr-4 space-y-4">
        {messages.map((msg, index) => (
          ...
          >
            {msg.sender === "ai" && (
              <div className="flex-shrink-0 w-8 h-8 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center text-amber-600">
                <SparklesIcon className="w-5 h-5" />
              </div>
            )}
            <div
              className={`max-w-md p-3 rounded-xl ${msg.sender === "ai" ? "bg-gray-100 dark:bg-gray-800" : "bg-amber-600 text-white"}`}
            >
      ...
      <form onSubmit={handleSendMessage} className="mt-4 flex-shrink-0">
        <div className="relative">
          ...
          <button
            type="submit"
            className="absolute right-3 top-1/2 -translate-y-1/2 text-amber-600 disabled:text-gray-400"
